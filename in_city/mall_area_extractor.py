...
"""
商场面状数据提取器
根据城市JSON文件中big_type为"购物服务"且mid_type为"商场"的POI，
获取对应商场的面状数据（边界几何信息）
"""
...
def extract_mall_pois(city_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """提取城市数据中big_type为"购物服务"且mid_type为"商场"的POI"""
    mall_pois = []
    city_name = city_data.get('city_name', '未知城市')
    
    print(f"正在提取 {city_name} 的商场POI...")
    print(f"检索条件: big_type='购物服务' AND mid_type='商场'")
    ...
    for hex_info in city_data.get('hexes', []):
        ...
        for poi in hex_info.get('pois', []):
            ...
            # 严格检查是否符合条件：big_type为"购物服务"且mid_type为"商场"
            if big_type == "购物服务" and mid_type == "商场":
                ...
                print(f"  找到商场: {poi.get('name', '未知商场')} (位置: {poi.get('lat', 0):.6f}, {poi.get('lng', 0):.6f})")
    
    print(f"在 {city_name} 中找到 {len(mall_pois)} 个符合条件的商场POI")
    return mall_pois


def extract_core_mall_name(poi_name: str) -> List[str]:
    """从复杂的商场名称中提取核心名称用于查询"""
    core_names = []
    
    # 移除常见的区域标识符
    cleaned_name = poi_name
    
    # 移除具体的区域、街道、楼层信息
    import re
    patterns_to_remove = [
        r'[A-Z]区\d*街?',      # A区、B区1街等
        r'[A-Z]\d*区\d*街?',   # A1区、B2区等
        r'[A-Z]座',            # A座、B座等
        r'\d+[层楼]',          # 1层、2楼等
        r'[东南西北]\d*[区街]', # 东区、南1街等
        r'[一二三四五六七八九十]\d*[区街层楼]',  # 一区、二街、三层等
    ]
    ...
    core_names.append(cleaned_name)
    
    # 添加原始名称作为备选
    if cleaned_name != poi_name:
        core_names.append(poi_name)
...
def get_poi_area_data(poi_name: str, poi_lat: float, poi_lng: float, city_name: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        # 提取核心商场名称
        core_names = extract_core_mall_name(poi_name)
        
        # 构建查询列表
        query_formats = []
        for core_name in core_names:
            if city_name:
                query_formats.append(f"{core_name}, {city_name}")
            query_formats.append(core_name)
        ...
        if gdf is None or gdf.empty:
            print(f"  无法找到 {poi_name} 的面状数据，尝试使用缓冲区...")
            # 如果找不到面状数据，创建一个小的缓冲区作为近似
            point = Point(poi_lng, poi_lat)
            # 创建500米的缓冲区（约0.0045度）
            buffer_area = point.buffer(0.0045)
            
            result = {
                'poi_name': poi_name,
                'geometry_type': 'buffered_point',
                'area_km2': 0.785,  # 500米半径圆的面积约0.785平方公里
                'bounds': buffer_area.bounds,
    ...
    for filename in os.listdir(json_dir):
        if filename.endswith('_h3_grid.json') and not filename.startswith('all_cities'):
            ...
            for i, poi in enumerate(mall_pois):
                print(f"  处理商场 {i+1}/{len(mall_pois)}: {poi.get('name', '未知商场')}")
